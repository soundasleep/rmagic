...
class PlayerTest < ActiveSupport::TestCase
  ...
  test "we can use green mana to pay for dual mana" do
  ...
  test "we can use partial mana pools" do
    player = Player.create({ mana_green: 2 })
    assert player.has_mana? ({ green: 1 })
  ...
  test "we can't use red mana to pay for green mana" do
    player = Player.create({ mana_red: 1 })
    assert !player.has_mana?({ green: 1 })
  end

  test "we can use green mana to pay for colourless mana" do
    player = Player.create({ mana_green: 1 })
  ...
  test "we have to have the right amount of mana for colourless mana" do
  ...
  test "we can use coloured mana to pay for colourless mana combos" do
    player = Player.create({ mana_green: 1, mana_blue: 2 })
    assert player.has_mana? ({ colourless: 2, green: 1 })
  ...
  test "we can use colourless mana to pay for colourless mana" do
