module Land
  include Playable

  def is_land?
    true
  end

  def can_tap?(game_engine, battlefield)
    return game_engine.duel.priority_player == battlefield.player &&
      game_engine.duel.phase.can_tap? &&
      battlefield.card.can_tap? &&
      battlefield.zone.cards_are_tappable?
  end

  def can_untap?(game_engine, battlefield)
    return false # we can never manually untap lands
  end

  def tap_cost(game_engine, battlefield)
    zero_mana
  end

  def untap_cost(game_engine, battlefield)
    zero_mana
  end

  def do_tap(game_engine, battlefield)
    fail "Cannot tap #{battlefield.card}: already tapped" if battlefield.card.is_tapped?

    battlefield.card.tap_card!
    battlefield.player.add_mana! mana_provided
  end

  def do_untap(game_engine, battlefield)
    fail "Cannot untap #{battlefield.card}: already untapped" if !battlefield.card.is_tapped?

    battlefield.card.untap_card!
  end

end
